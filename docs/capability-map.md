...
|---|---|---|---|---|---|
| **Gmail** | Google OAuth 2.0 (user consent) | List/search threads, read message content and metadata | Create draft, send message, apply labels | Google API Python client (`google-api-python-client`) wrapped as a typed tool with explicit capability flags; can be re-exposed as an MCP server later without changing the tool interface | Sending is irreversible — gated behind approval always. Needs a Google Cloud OAuth client (one-time setup, credentials never committed). |

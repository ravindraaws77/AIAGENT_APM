...
| **Google Calendar** | Google OAuth 2.0 (same consent screen as Gmail, can share scopes) | **Implemented (phase 3):** list/search events, read attendees/times/location — `src/apm/tools/calendar_tool.py` | Create event, update event, respond to invite — **not yet implemented.** Deferred to phase 5, same reasoning as Gmail's send capability | Same Google API client library as Gmail (`GoogleApiCalendarClient`, same `google_auth.py` helper); a `CalendarClient` protocol lets tests substitute a fake client with no live credentials | Time zone / recurring-event handling adds complexity — out of scope for MVP (single, non-recurring events only). Also: today the Gmail and Calendar demo scripts request separate OAuth scopes against the same token cache, so running both re-prompts for consent — phase 5 should request both scopes in one `load_credentials` call when wiring the agent (see the note in `google_auth.py`). |
| **MS Excel** | Microsoft identity platform (Azure AD app registration), MSAL device-code flow, Microsoft Graph API | **Implemented (phase 4):** list worksheets, read a cell range — `src/apm/tools/excel_tool.py` | Write/update cell ranges, append table rows — **not yet implemented.** Deferred to phase 5, same reasoning as Gmail's send / Calendar's create-event | Plain `requests` against the Graph REST API (`GraphApiExcelClient`), wrapped with the same tool interface as Gmail/Calendar; an `ExcelClient` protocol lets tests substitute a fake client with no live credentials | Needs a separate Azure AD app registration (different admin/consent flow than Google) — this is the reason Excel is its own phase rather than bundled with the Google tools. Workbook must already be on OneDrive/SharePoint (Graph API doesn't operate on local `.xlsx` files). Unlike Gmail/Calendar, the connector is bound to one specific workbook (drive item id) at construction time rather than operating on "the" mailbox/calendar — `scripts/excel_demo.py --list` helps find that id. |

...
| **Excel files (local + Google Drive)** | Local: none. Google Drive: Google OAuth 2.0, `drive.file` scope (same client as Gmail/Calendar, separate token cache) | **Implemented:** list worksheets, read a cell range — `src/apm/tools/excel_file_tool.py` | **Implemented:** overwrite a cell range (`write_range`) — same dry-run-gated pattern as Gmail/Calendar/MS Excel, not yet wired into the agent graph | Complements the Graph-based MS Excel connector above for workbooks that are plain `.xlsx` files rather than something already living on OneDrive/SharePoint: a `WorkbookSource` protocol (`LocalWorkbookSource` for a path on disk, `GoogleDriveWorkbookSource` downloading/uploading file bytes via the Drive v3 API) supplies whole-workbook bytes, which `ExcelFileTool` parses/edits with `openpyxl` — there's no cell-range API on either source, unlike Graph's Excel API. Same `ExcelClient`-shaped test-fake pattern (`FakeWorkbookSource`) as the other connectors. | Local source needs no credentials at all. Drive source only handles an actual `.xlsx` uploaded to Drive, not a native Google Sheet (Sheets is a different file type with no `.xlsx` bytes to download — would need the separate Sheets v4 API, out of scope here) — bound to one Drive file id at construction time, same as MS Excel's workbook binding. `read_range` loads with `data_only=True` to return each formula cell's last-*calculated* value; a workbook that's never been opened in real Excel/Sheets (e.g. one only ever written by openpyxl) has no cached value, so such cells read back as `None` — not a concern for `write_range`, which reloads without `data_only` so it never clobbers other cells' formulas when saving. Not yet wired into `scripts/agent_demo.py` or the agent graph, same status as MS Excel's `write_range`. |
